...
if __name__ == "__main__":
    ...
    with mlflow.start_run() as run:
        ...
        # Note: some MLflow tracking servers (for example DagsHub's hosted
        # implementation) may not support the newer `CreateLoggedModel`
        # endpoint used by `mlflow.sklearn.log_model`. That can cause a
        # RestException: 'unsupported endpoint'. To remain compatible we
        # log the serialized model file as a regular artifact instead of
        # using `log_model` which attempts to call registry endpoints.
        # Save model artifact
        mlflow.log_artifact(str(model_path))
        logger.info("Model file logged as artifact")
